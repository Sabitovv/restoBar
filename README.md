...
Backend now includes production-ready foundations:
- PostgreSQL models and Alembic migrations (`backend/app/models.py`, `backend/migrations/`)
- Redis integration and idempotency primitives (`backend/app/services/idempotency_service.py`, `backend/app/services/lock_service.py`)
- Dual-write order persistence path (`backend/app/services/storage_service.py`)
- Payment state transition persistence (`backend/app/services/payment_service.py`)
- AI reply service scaffold and worker task (`backend/app/services/ai_service.py`, `backend/app/workers/ai_tasks.py`)
- Observability hooks (`/metrics`, correlation id middleware, optional Sentry init)

### Rollout and rollback playbook

1. Apply DB schema: run `alembic upgrade head` in `backend`.
2. Seed menu data: run `python scripts/seed_json_to_pg.py`.
3. Start in dual-write mode: keep `DUAL_WRITE_ORDERS=1` and `WRITE_ORDERS_TO_JSON=1`.
4. Check consistency: run `python scripts/consistency_checks.py`.
5. Cut over reads/writes fully to PostgreSQL after validation period.
6. Disable JSON mirror: set `WRITE_ORDERS_TO_JSON=0`.

Rollback:
- If an issue appears before full cutover, keep PostgreSQL writes enabled and temporarily re-enable JSON-based fallback reads.
- Do not roll back paid orders from PostgreSQL; treat PostgreSQL as financial source once payment flow is live.

### Quick start (new backend stack)
...
4. Seed menu to PostgreSQL:
   - `python scripts/seed_json_to_pg.py`
5. Start backend:
   - `flask --app app.main:app run`

For full local container run, use `docker compose up --build`.
...
`ADMIN_DEV_APP_URL` - local URL of React admin panel (for example `http://localhost:5173`) added to CORS in `DEV_MODE`.
...
`BOT_MODE` - `webhook` for public HTTPS webhook mode, `polling` for local development without webhook.
...
`DATABASE_URL` - PostgreSQL DSN used by SQLAlchemy and Alembic.
`REDIS_URL` - Redis DSN used for locks, deduplication and queue.
`ENABLE_AI_BOT_REPLIES` - enables AI responses in bot fallback handler.
`AI_API_KEY` - OpenAI API key for real AI responses.
`AI_MODEL` - model name for AI responses.
`AI_REQUEST_TIMEOUT_SECONDS` - timeout for AI request execution.
`AI_MAX_RETRIES` - retry count for AI response generation.
`RQ_QUEUE_NAME` - Redis queue name for background AI tasks.
`DUAL_WRITE_ORDERS` - writes orders to PostgreSQL in checkout flow.
`WRITE_ORDERS_TO_JSON` - keeps JSON mirror during migration window.
`ORDERS_JSON_PATH` - target JSON file for temporary order mirror.
`READ_MENU_FROM_PG` - read categories/menu/details from PostgreSQL first.
`JSON_MENU_FALLBACK` - fallback to JSON menu data when PostgreSQL returns empty data.
`SENTRY_DSN` - optional Sentry DSN.
`SENTRY_TRACES_SAMPLE_RATE` - tracing sampling for Sentry.
`ADMIN_SESSION_SECRET` - secret used to sign admin auth session token (fallbacks to `BOT_TOKEN`).

### Admin panel (React + Vite)

1. Create env and run admin web app:
...
2. Configure required values in `admin-web/.env`:
   - `VITE_API_BASE_URL=http://localhost:5000`

### Admin panel auth bootstrap (pre-approved only)

Admin login is WebApp-only and uses Telegram Mini App `initData` payload verification. Access is allowed only for users that exist in `staff_memberships` with active status.

After migrations, create first super admin membership:

```shell
cd backend
python scripts/bootstrap_super_admin.py --telegram-user-id <TELEGRAM_USER_ID> --username <telegram_username>
```

Admin auth endpoint:

- `POST /admin/auth/webapp`
- `POST /admin/auth/telegram` (optional widget fallback)
- `GET /admin/restaurants`
- `POST /admin/restaurants` (super admin only)
- `DELETE /admin/restaurants/<restaurant_id>` (super admin only, sets restaurant inactive)
- `GET /admin/staff`
- `POST /admin/staff/invite`
- `PATCH /admin/staff/<membership_id>/role`
- `DELETE /admin/staff/<membership_id>`
- `GET /admin/menu/categories`
- `POST /admin/menu/categories`
- `GET /admin/menu/items`
- `POST /admin/menu/items`
- `PATCH /admin/menu/items/<menu_item_id>/availability`

Telegram bot role-aware entry:

- `/start` sends `Open Admin` button for active staff (`super_admin/admin/manager`) and `Explore Menu` for regular customers.
- `/admin` command opens admin panel only for active staff users.
...
For local bot development without a public HTTPS URL, set `BOT_MODE=polling` and run:

```shell
python -m app.run_bot_polling
```

Quick mode switch commands:

```shell
# polling mode (local bot, no public webhook)
cp backend/.env.polling backend/.env

# webhook mode (public HTTPS URL required)
cp backend/.env.webhook backend/.env
```
